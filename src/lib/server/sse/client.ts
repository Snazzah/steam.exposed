...
export class SSEConnection extends EventEmitter {
	...
	send(message: AnyMessageEvent) {
		if (this.state === ConnectionReadyState.CLOSED)
			throw new Error('Tried to send message to a closed connection');
	...
	#log(message: string, ...args: any[]) {
		console.debug(`[SSE_CONNECTION:${this.id}] ${message}`, ...args);
	}
}
