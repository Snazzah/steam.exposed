...
import {
	ACH_DATA_STALE_TIME,
	GAME_ACH_INCOMPLETE_STALE_TIME,
	GAME_ACH_STALE_TIME,
	PLAYER_ACH_INCOMPLETE_STALE_TIME,
	YEAR_ACH_STALE_TIME
} from './util';
...
async function worker(task: Task, cb: done) {
	switch (task.type) {
		...
		case 'fetchGameAchievements': {
			logQueue(
				`refetching game achievements of appid ${task.appid} for steamid ${task.steamid ?? '<none>'} (reason: ${task.reason ?? '<unk>'})`
			);
		...
		case 'fetchPlayerAchievements': {
			logQueue(
				`refetching player achievements of appid ${task.appid} for steamid ${task.steamid} for year ${task.year} (reason: ${task.reason ?? '<unk>'})`
			);
		...
		case 'fetchUserYearAchievements': {
			logQueue(
				`refetching user year achievements for steamid ${task.steamid} for year ${task.year} (reason: ${task.reason ?? '<unk>'})`
			);
...
async function achWorker(task: AchievementTask, cb: done) {
	...
	try {
		...
		if (
			new Date(yearAchievements._fetchedAt).getFullYear() === year &&
			Date.now() - yearAchievements._fetchedAt > YEAR_ACH_STALE_TIME
		)
			q.push({
				type: 'fetchUserYearAchievements',
				steamid: id,
		...
		let appsFetched = 0;
		const appAmount = Object.entries(yearAchievements.apps).filter(
			([, achs]) => achs.achievements.length !== 0
		).length;
		for (const appid in yearAchievements.apps) {
			...
			task.send({
				event: 'update',
				data: {
					text: `Fetching game achievements (${++appsFetched}/${appAmount}): ${appid}`,
					progress: appsFetched / (appAmount * 2)
				}
			});
			...
			if (
				gameAchievements &&
				Date.now() - gameAchievements._fetchedAt >
					(gameAchievements.complete ? GAME_ACH_STALE_TIME : GAME_ACH_INCOMPLETE_STALE_TIME)
			)
				q.push({
					type: 'fetchGameAchievements',
		...
		for (const appid in yearAchievements.apps) {
			...
			task.send({
				event: 'update',
				data: {
					text: `Fetching player achievements (${++playerAchsFetched}/${appAmount}): ${appid}`,
					progress: (appsFetched + playerAchsFetched) / (appAmount * 2)
				}
			});
			...
			if (
				playerAchievements?.success === false &&
				Date.now() - playerAchievements._fetchedAt > PLAYER_ACH_INCOMPLETE_STALE_TIME
			)
				q.push({
					type: 'fetchPlayerAchievements',
					appid: parseInt(appid, 10),
			...
			allPlayerAchievements[appid] = playerAchievements.success
		...
		await redis.set(
			`achdata:${task.steamid}:${task.year}`,
			JSON.stringify(achData),
			'EX',
			ACH_DATA_STALE_TIME / 1000
		);
