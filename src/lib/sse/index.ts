...
export class SSEClient extends Emittery<EmitterEvents> {
	...
	#bindMap: Record<
		string,
		((event: MessageEvent<any>) => Promise<void> | void) | keyof EmitterEvents
	> = {
		open: this.#onOpen,
		error: this.#onError,
	...
	constructor() {
		...
		for (const event in this.#bindMap) {
			...
			if (typeof binding === 'string')
				this.#bindings[event] = this.#forwardEvent.bind(this, binding);
	...
	#close(notify = false) {
		if (this.source) {
			for (const event in this.#bindings)
				this.source.removeEventListener(event, this.#bindings[event]);
			this.source.close();
