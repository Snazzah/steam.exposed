<script lang="ts">
		...
		const { completed } = calcAchievements(
			...
			allAchievements.filter(
				(a) =>
					a.appid === appid &&
					a.unlocked !== 0 &&
					a.unlocked >= startDate / 1000 &&
					a.unlocked < stopDate / 1000
			)
		);
		return completed;
	});
	...
	const months = yearInReview.stats.playtime_stats.months.map((m, i) => ({
		...
		achievements: allAchievements
			.filter(
				(ach) =>
					ach.unlocked >= m.rtime_month &&
					(yearInReview.stats.playtime_stats.months[i + 1]?.rtime_month ?? stopDate / 1000) >
						ach.unlocked
			)
			.sort((a, b) => a.unlocked - b.unlocked)
	}));
	const days = getCalendarRange(
		new Date(`Jan 1 ${yearInReview.stats.year}`),
		new Date(`Dec 31 ${yearInReview.stats.year}`)
	);
	...
	function getMonthColumn(month: number) {
		return (
			(Math.round(days.indexOf(calendar[month].find((d) => d.getDay() === 0)!)) +
				calendar[0][0].getDay()) /
			7
		);
	}
	...
	let selectedGames = $derived(selected ? groupBy(selected.achs, (a) => a.appid) : {});
...
<div class="flex flex-col gap-8">
	<div class="flex gap-8 justify-around flex-wrap">
		...
		<BigStat
			...
			subtext={`(${(
				(achievements.gamesWithAchievements /
					yearInReview.stats.playtime_stats.game_summary.length) *
				100
			).toFixed(2)}%)`}
		>
			{achievements.gamesWithAchievements.toLocaleString()}
		</BigStat>
		<BigStat name="Completed Games This Year">
			{completedGamesThisYear.length.toLocaleString()}
		</BigStat>
	</div>
	<div
		class="grid grid-rows-7 w-fit gap-px max-w-full overflow-x-auto mx-auto pb-2 pt-6 relative select-none"
	>
		<!-- Month names -->
			...
			<button
				...
				onclick={() =>
					(selected = {
						...
						achs: months[month].achievements
					})}
			>
				{monthName}
			</button>
		...
		{#each days as date, i}
			...
			{@const achs = allAchievements.filter(
				(a) => date.valueOf() <= a.unlocked * 1000 && date.valueOf() + 86400000 > a.unlocked * 1000
			)}
			...
			<button
				...
				onclick={() => (selected = { type: 'day', date, achs })}
			>
				{plus ? '+' : achs.length || ''}
			</button>
		{/each}
	...
	{#if !achievements.complete}
		<div
			class="flex gap-2 md:items-center flex-col md:flex-row px-4 py-2 rounded bg-red-900/50 text-red-100"
		>
		...
		<div class="flex flex-col gap-8">
			...
			<div>
				<h3 class="font-bold text-white text-2xl">
					{(selected.type === 'month' ? longMonthDtf : dayDtf).format(selected.date)}
				</h3>
				<span
					>{plural(selected.achs.length, 'achievement')} over {plural(
			...
			{:else}
				{#each Object.keys(selectedGames) as appidStr (`${selected.date}:${appidStr}`)}
					...
					{@const { achTotal, achEarned, achEarnedBefore, earnedPercent, completed } =
						calcAchievements(achievements, appid, selectedGames[appid])}
					...
					<div class="flex flex-col gap-2">
						<div class="flex gap-4 items-center flex-none flex-col sm:flex-row">
							...
							>
								...
								{#if completed}
									<Icon
										icon={perfectGameIcon}
										class="absolute -bottom-5 -left-5 w-10 h-10 pointer-events-none"
									/>
								{/if}
							</button>
