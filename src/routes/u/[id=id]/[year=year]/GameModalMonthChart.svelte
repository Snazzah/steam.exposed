<script lang="ts">
	...
	const playtimeSeconds = moreInfo
	...
	const data = yearInReview.stats.playtime_stats.months.map((m) => ({
		month: m.rtime_month,
		value:
			(m.game_summary.find((g) => g.appid === appId)?.total_playtime_percentagex100 ?? 0) /
			game.total_playtime_percentagex100
	}));
...
<div class="flex flex-col gap-4">
	...
	<div
		class={`rounded-md px-4 py-2 transition-colors ${
			selectedMonthIndex === -1 ? 'bg-black/50' : 'bg-neutral-700'
		}`}
	>
		...
		{:else}
			...
			{#if data[selectedMonthIndex].value}
				{(data[selectedMonthIndex].value * 100).toFixed(2)}% of total game playtime, ~{prettyMilliseconds(
					playtimeSeconds * data[selectedMonthIndex].value * 1000
				)}
			{:else}
