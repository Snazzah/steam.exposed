<script lang="ts">
	...
	interface Props {
		...
		dayDtf: Intl.DateTimeFormat;
		onclick?(): void;
	}

	let { achievement, gameName = '', unlockedAt = 0, dtf, dayDtf, onclick }: Props = $props();
...
<button
	...
	use:floatingRef
	onmouseenter={() => showTooltip.set(1, { duration: 100 })}
...
>
	<img
		class={achievement.hidden
			? 'blur-sm group-hover:blur-0 group-focus:blur-0 grayscale group-hover:grayscale-0 group-focus:grayscale-0 transition-all w-12 h-12 object-contain'
			: 'transition-all w-12 h-12 object-contain pix'}
				...
				<div class="text-xs sm:text-sm mt-2 text-neutral-400 flex flex-col">
					...
					{#if achievement.removedAt}
						<span class="text-red-400"
							>This achievement was removed some time before {dayDtf.format(
								achievement.removedAt
							)}</span
						>
					{/if}
				</div>
...
<style lang="scss">
	._glow {
		box-shadow:
			rgba(255, 184, 78, 0.6) 0px 0px 2px 1px,
			rgba(255, 184, 78, 0.4) 0px 0px 16px 1px;
	}
</style>
