<script lang="ts">
	...
	interface Props {
		...
		achTotal: number;
		achEarned: number;
		achEarnedBefore: number;
		completed: boolean;
		earnedPercent: number;
	}

	let { achAmount, achTotal, achEarned, achEarnedBefore, completed, earnedPercent }: Props =
		$props();
</script>
...
<div class="flex justify-between font-medium">
	...
	{#if achTotal !== 0}
		<div
			class={completed
				? 'flex gap-1 text-white justify-center items-center'
...
{#if achTotal !== 0}
	<div class="relative rounded-full bg-neutral-700 w-full h-2 overflow-hidden">
		<div
			class="h-full bg-blue-400 absolute left-0"
			style:width={`${((achEarned + achEarnedBefore) / achTotal) * 100}%`}
		></div>
		<div
			class="h-full bg-blue-600 absolute left-0"
			style:width={`${(achEarnedBefore / achTotal) * 100}%`}
		></div>
	</div>
{/if}
