...
<div class="flex gap-8 justify-around flex-wrap">
	<BigStat
		name="Total Playtime"
		subtext={`(${((totalStats.total_playtime_seconds / 31536000) * 100).toFixed(
			2
		)}% of an entire year)`}
	>
	...
	<BigStat
		...
		subtext={`(${summaryStats.total_rare_achievements.toLocaleString()} rare achievements, ${(
			(summaryStats.total_rare_achievements / summaryStats.total_achievements) *
			100
		).toFixed(2)}%)`}
	>
