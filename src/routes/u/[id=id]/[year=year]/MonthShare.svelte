...
{#if $page.data.profile}
  ...
  >
    <Icon icon={rendering ? loadingIcon : screenshotIcon} class={`w-6 h-6${rendering ? ' animate-spin' : ''}`} />
  ...
  {#if rendering}
    ...
    <div class="w-0 h-0 overflow-hidden absolute">
      <div class="w-[800px] h-[400px] relative bg-neutral-950 flex flex-col pointer-events-none select-none" bind:this={chart}>
        ...
        <div class="flex justify-around bg-black/25 h-6 leading-[0] pt-1 text-sm border-t border-white/10">
          <span><span class="text-white">{prettyMilliseconds(playtimeStats.total_stats.total_playtime_seconds * 1000, { compact: true, verbose: true })}</span> of playtime</span>
          <span><span class="text-white">{playtimeStats.game_summary.length.toLocaleString()}</span> games</span>
          <span><span class="text-white">{playtimeStats.total_stats.total_sessions.toLocaleString()}</span> sessions</span>
          <span><span class="text-white">{playtimeStats.summary_stats.total_achievements.toLocaleString()}</span> achievements</span>
        </div>
        <div class="p-4">
        <!-- <div class="px-4 py-8"> -->
