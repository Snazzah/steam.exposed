		...
		<div class="flex flex-col w-full justify-end">
			...
			>
				<div
					...
					style:height={`${m.value * 100 * (1 / (topPercentage / 100))}%`}
				>
					{#if m.value}
						<span
							class="absolute w-full text-center text-neutral-200 group-hover:text-white -top-4 hidden md:block"
						>
							{useInt ? m.value : `${(m.value * 100).toFixed(2)}%`}
						</span>

						<div
							class="absolute flex flex-col w-full top-0 bottom-0 overflow-hidden rounded"
							class:hidden={chartType !== 0}
						>
							{#each month.game_summary.sort((a, b) => b.relative_playtime_percentagex100 - a.relative_playtime_percentagex100).slice(0, 3) as game, i (`${game.appid}:${m.month}`)}
								<div
									class={`relative overflow-hidden ${gameColorClasses[i]}`}
									style:height={`${game.relative_playtime_percentagex100 / 100}%`}
								>
						...
						<div
							class="absolute flex flex-col w-full top-0 bottom-0 overflow-hidden rounded"
							class:hidden={chartType !== 1}
						>
							<div
								class="group-hover:brightness-110"
								style:background-color="#d67070"
								style:height={`${
			...
			<span
				class="w-full text-center h-5 pt-1 tracking-tighter whitespace-nowrap"
				class:text-white={selectedIndex === i}
				class:font-bold={selectedIndex === i}
			>
