...
const REQUIRED_PARAMS = [
  'openid.ns', 'openid.mode', 'openid.op_endpoint', 'openid.claimed_id', 'openid.identity', 'openid.return_to',
  'openid.response_nonce', 'openid.assoc_handle', 'openid.signed', 'openid.sig'
]

export const GET: RequestHandler = async ({ url, cookies }) => {
  if (REQUIRED_PARAMS.map((p) => url.searchParams.has(p)).includes(false)
    ...
    || !url.searchParams.get('openid.claimed_id')!.startsWith('https://steamcommunity.com/openid/id/')
